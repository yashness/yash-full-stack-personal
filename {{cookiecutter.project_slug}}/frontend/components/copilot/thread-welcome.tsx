...
/**
 * Suggestion button that triggers a prompt.
 */
function SuggestionButton({ icon, title, description, prompt, onClick }: SuggestionButtonProps) {
  return (
    <button
      onClick={() => onClick?.(prompt)}
      className="flex flex-col items-start gap-2 p-4 bg-card border border-border rounded-xl hover:bg-muted hover:border-primary/50 transition-all cursor-pointer group text-left w-full"
    >
 ...
 */
export function ThreadWelcome({ onSuggestionClick }: ThreadWelcomeProps = {}) {
  return (
